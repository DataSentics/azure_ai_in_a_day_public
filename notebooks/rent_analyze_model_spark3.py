...
# Plot histogram of totalRent
display(df)

# COMMAND ----------

# Plot histogram of totalRent less than 10000 EUR
display(
  df.filter(F.col('totalRent') < 10000)
)

# COMMAND ----------

# Plot rent per yearConstructed
display(
  df
  .filter(F.col('totalRent') < 10000)
  .filter(F.col('yearConstructed').between(1900, 2020))
)

# COMMAND ----------

# Plot average totalRent per regio1
display(
  df.filter(F.col('totalRent') < 10000)
)
...
string_indexer = StringIndexer(inputCols=categorical_features, outputCols=[f'{col}_si' for col in categorical_features])
...
pipeline_lr = Pipeline(stages=[string_indexer, ohe_estimator, vector_assembler, model_lr])
pipeline_glm = Pipeline(stages=[string_indexer, ohe_estimator, vector_assembler, model_glm])
pipeline_rf = Pipeline(stages=[string_indexer, ohe_estimator, vector_assembler, model_rf])
...
# scatter totalRent vs. prediction
display(df_predictions_lr)
